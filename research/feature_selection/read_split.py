...
class ReadSplit(object):
    ...
    def split_dataset(self, dataset, sampling_type=3):
	    ...
	    # Last column is assumed to be dependent variable, change as needed
            y = dataset.iloc[:, -1]
            count_predictors = Counter(y)

            # Checking for class imbalance if samples are not equally distributed, utilize synthetic or other sampling techniques.
	    #print(" Checking class balance : ", count_predictors)
            #print("count predictors percentage :")
            for i in count_predictors:
            	print("class"+str(i)+" = ", (count_predictors[i]/len(dataset))*100)
